@extends('layouts.app')
@section('title','Maisonneuve - ' .trans('lang.text_heading_update_my_post'))
@section('heading-title', trans('lang.text_heading_update_my_post'))
                    ...
                    <div class="tab-content border border-top-0" id="myTabContent">
                        <div class="tab-pane fade show active" id="english" role="tabpanel" aria-labelledby="english-tab">
                            <div class="my-3">
                                ...
                                <input type="text" class="form-control @if ($errors->has('titre_en')) border-danger @endif" id="titre_en" name="titre_en" value="{{ $post->titre_en }}">
                            </div>
                            <div class="mb-3">
                                <label for="contenu_en" class="form-label">@lang('lang.text_content_en')</label>
                                <textarea class="form-control @if ($errors->has('contenu_en')) border-danger @endif" id="contenu_en" name="contenu_en" rows="6">{{ $post->contenu_fr }}</textarea>
                            </div>
                        </div>
                        <div class="tab-pane fade" id="french" role="tabpanel" aria-labelledby="french-tab">
                            <div class="my-3">
                                ...
                                <input type="text" class="form-control @if ($errors->has('titre_fr')) border-danger @endif" id="titre_fr" name="titre_fr" value="{{ $post->titre_fr }}">
                            </div>
                            <div class="mb-3">
                                <label for="contenu_fr" class="form-label">@lang('lang.text_content_fr')</label>
                                <textarea class="form-control @if ($errors->has('contenu_en')) border-danger @endif" id="contenu_en" name="contenu_fr" rows="6">{{ $post->contenu_fr }}</textarea>
                            </div>
                        </div>
                    </div>
                </div>
